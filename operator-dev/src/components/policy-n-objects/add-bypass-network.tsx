...
export const AddBypassNetwork: FC<props> = ({ visible, setVisible, refresh }) => {
    ...
    const add = (network: string) => {
        apiInstance.api.getConfig()
            .then(res => res.data)
            .then(res => {
                let bypass_network_temp = res.bypass_networks!
                bypass_network_temp = !bypass_network_temp ? [] : bypass_network_temp
                bypass_network_temp.push(network)
                let conf_temp = res
                conf_temp.bypass_networks = bypass_network_temp
    ...
    return (
        <div>
            <Dialog header="Add network to bypass a captive portal " visible={visible} style={{ width: '530px', height: 'auto' }} onHide={() => {
                setVisible(false)
            }}>
